...
import (
	"context"
	client2 "github.com/Myriad-Dreamin/go-ves/central-ves/web-socket/client"
...
func (srv *Server) serveWs(w http.ResponseWriter, r *http.Request) {
	conn, err := client2.Upgrader.Upgrade(w, r, nil)
	if err != nil {
	...
	c := &client2.Client{
		Hub: srv.hub, Closed: atomic.NewBool(false), Helloed: make(chan bool, 1), Send: make(chan *hub.WriteMessageTask, 256)}
