...
func (g *getter) GetTransactionProof(chainID uip.ChainID, blockID uip.BlockID, color []byte) (uip.MerkleProof, error) {
	panic("implement me")
}

func (g *getter) GetStorageAt(chainID uip.ChainID, typeID uip.TypeID, contractAddress uip.ContractAddress, pos []byte, description []byte) (uip.Variable, error) {
	fmt.Println("getting", chainID, typeID, contractAddress, pos, description)
	return uip.VariableImpl{typeID, true}, nil
}
